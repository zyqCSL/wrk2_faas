...
void *thread_main(void *arg) {
    ...
    double throughput = (thread->throughput / 1000000.0) / thread->connections; // requests per us per connection
...
static int parse_args(struct config *cfg, char **url, struct http_parser_url *parts, char **headers, int argc, char **argv) {
    ...
    while ((c = getopt_long(argc, argv, "t:c:d:s:u:D:H:T:R:LPpBrv?", longopts, NULL)) != -1) {
        switch (c) {
            case 't':
            ...
            case 'T':
                ...
                break;
            case 'R':
                if (scan_double_metric(optarg, &cfg->rate)) return -1;
                break;
            case 'v':
