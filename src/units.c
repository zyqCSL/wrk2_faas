...
static int scan_double_units(char *s, double *n, units *m) {
    double base = 1;
    uint64_t scale = 1;
    char unit[3] = { 0, 0, 0 };
    int i, c;

    if ((c = sscanf(s, "%lf%2s", &base, unit)) < 1) return -1;
...
int scan_double_metric(char *s, double *n) {
    return scan_double_units(s, n, &metric_units);
}
